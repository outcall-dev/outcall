...
blockers found after v0.1.9. It supersedes the failed v0.1.10 through v0.1.14
tag attempts, which did not include the final privileged CI harness fixes.
...
- Privileged agent API tests now send explicit close semantics and assert
  status codes against full raw HTTP responses, avoiding empty-body false
  failures from status-only responses.
- Privileged CLI integration tests now wait for the daemon socket, surface
  startup stderr, and run with `--no-proxy` because those checks only exercise
  the host socket API.
- Non-privileged CLI parsing coverage no longer starts the daemon just to test
